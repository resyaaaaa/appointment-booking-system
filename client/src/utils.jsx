...
export function formatHumanDate(dateStr) {
  ...
  // Create a Date in UTC to keep it timezone-agnostic and avoid day alignment slippage
...
// Convert creation date ISO string to a beautiful date and time in Malaysia Time (MYT), e.g., "Jun 11, 2026 at 07:15 PM"
export function formatHumanDateTime(isoStr) {
  ...
  return `${dateFormatted} at ${timeFormatted} (MYT)`;
}
...
export function formatUSD(value, currency = 'RM') {
  ...
  // Format cleanly with any other currency code / symbol input e.g. "£", "$", "EUR"
  try {
    return new Intl.NumberFormat('en-US', {
      style: 'currency',
